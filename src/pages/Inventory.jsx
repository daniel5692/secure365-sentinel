...
import { Users, Mail, Building2, AppWindow, RefreshCw, AlertTriangle, CheckCircle2, XCircle, Key, BookUser, Trash2, UserCheck, ArrowRight } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      {data && (
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
          {CATEGORIES.map(cat => {
            ...
            return (
              <button
                ...
                className={cn(
                  "bg-card border rounded-xl p-5 text-right transition-all group cursor-pointer",
                  c.border, c.hover,
                  "hover:bg-secondary/30 hover:shadow-lg hover:scale-[1.02]"
                )}
              >
                ...
                {cat.key === 'appCredentials' && data.appCredentials && (
                  <div className="mt-2 flex gap-2 text-[10px]">
                    ...
                    {data.appCredentials.filter(a => a.status === 'expiring_soon').length > 0 && (
                      <span className="px-1.5 py-0.5 rounded bg-amber-500/10 text-amber-400 border border-amber-500/30">
                        {data.appCredentials.filter(a => a.status === 'expiring_soon').length} פגים בקרוב
                      </span>
                    )}
                  </div>
                )}
          ...
          {/* Total users card (no detail) */}
          <div className={cn("bg-card border rounded-xl p-5 text-right", COLORS.default.border)}>
            <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center mb-4", COLORS.default.bg)}>
              <Users className={cn("w-5 h-5", COLORS.default.text)} />
            </div>
            <div className={cn("text-3xl font-bold mb-1", COLORS.default.text)}>{data.stats?.totalUsers ?? '—'}</div>
            <div className="text-sm text-muted-foreground">סה״כ חשבונות</div>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        {(category.key === 'activeMembers' || category.key === 'guestUsers' || category.key === 'sharedMailboxes' || category.key === 'deletedUsers') && (
          <UserDetailTable users={items} showDeleted={category.key === 'deletedUsers'} />
        )}
        {category.key === 'rooms' && <RoomsDetail rooms={items} />}
        {category.key === 'contacts' && <ContactsDetail contacts={items} />}
        {category.key === 'appCredentials' && <AppCredsDetail apps={items} />}
      </div>
    </div>
  );
}

function UserDetailTable({ users, showDeleted }) {
  if (users.length === 0) return <EmptyState />;
  return (
    <div>
      <div className="grid grid-cols-12 gap-4 px-5 py-3 bg-secondary/30 text-[11px] font-semibold text-muted-foreground border-b border-border">
        <div className="col-span-1"></div>
        <div className="col-span-4">שם</div>
        <div className="col-span-4">UPN</div>
        <div className="col-span-3">{showDeleted ? 'תאריך מחיקה' : 'כתובת מייל'}</div>
      </div>
      <div className="divide-y divide-border max-h-[600px] overflow-y-auto">
        {users.map(u => (
          <div key={u.id} className="grid grid-cols-12 gap-4 px-5 py-3 text-xs items-center hover:bg-secondary/20">
            <div className="col-span-1">
              <span className={cn("w-2 h-2 rounded-full block", u.accountEnabled === false ? "bg-slate-500" : "bg-green-400")} />
            </div>
            <div className="col-span-4 font-medium text-foreground">{u.displayName}</div>
            <div className="col-span-4 text-muted-foreground font-mono truncate">{u.userPrincipalName}</div>
...
function ContactsDetail({ contacts }) {
  ...
  return (
    <div>
      ...
      <div className="divide-y divide-border max-h-[600px] overflow-y-auto">
        {contacts.map(c => (
          <div key={c.id} className="grid grid-cols-12 gap-4 px-5 py-3 text-xs items-center hover:bg-secondary/20">
            <div className="col-span-4 font-medium text-foreground">{c.displayName}</div>
            <div className="col-span-4 text-muted-foreground">{c.emailAddresses?.[0]?.address || '—'}</div>
            <div className="col-span-2 text-muted-foreground">{c.companyName || '—'}</div>
            <div className="col-span-2 text-muted-foreground">{c.jobTitle || '—'}</div>
          </div>
        ))}
      </div>
    </div>
        ...
        return (
          <div key={app.id} className="px-5 py-4 hover:bg-secondary/20">
            <div className="flex items-center gap-3 mb-2">
              <div className={cn("flex items-center gap-1.5 px-2 py-0.5 rounded border text-[11px] font-medium", s.bg, s.color)}>
                <StatusIcon className="w-3 h-3" />
                {s.label}
                {app.soonExpiringCount > 0 && ` (${app.soonExpiringCount})`}
              </div>
              <span className="text-xs font-semibold text-foreground">{app.displayName}</span>
              <span className="text-[10px] text-muted-foreground font-mono mr-auto">{app.appId}</span>
            </div>
            {app.credentials.length > 0 && (
              <div className="flex flex-wrap gap-2 mr-1">
                {app.credentials.map((c, i) => (
                  <span key={i} className={cn(
                    "flex items-center gap-1 px-2 py-1 rounded border text-[11px]",
                    ...
                    {c.type === 'secret' ? '🔑' : '📜'}
                    {c.displayName && <span className="max-w-[120px] truncate">{c.displayName}</span>}
                    <span>{c.isExpired ? `פג לפני ${Math.abs(c.daysLeft)} ימים` : `${c.daysLeft} ימים`}</span>
                  </span>
                ))}
              </div>
            )}
...
function EmptyState() {
  return (
    <div className="p-12 text-center text-sm text-muted-foreground">אין נתונים להצגה</div>
  );
}